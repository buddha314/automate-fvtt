...
Define how a Keep maintains a roster of member Actors and how content-registered
benefit definitions are resolved, applied, queried, and invoked against those
members. The engine stays game-system-agnostic: it owns membership, eligibility,
condition gating, idempotent application/revocation, and the public query/action
surface, while content supplies the system-specific semantics.
